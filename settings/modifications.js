...
fetch("/config")
	...
	.then(fetchedConfig => {
		...
		Object.keys(config.buttonsSettings).forEach(buttonKey => {
			...
			div.innerHTML = `
              <h2>${buttonKey}</h2>
              <label>Name: <input type="text" name="${buttonKey}-name" value="${
				button.name
			}"></label><br>
              <label>Background Color: <input type="color" name="${buttonKey}-backgroundColor" value="${
				button.backgroundColor
			}"></label><br>
            <label>Background Image: 
              <input type="file" name="${buttonKey}-backgroundImage" accept="image/*">
              </label>
              <img id="${buttonKey}-preview" src="${
				button.backgroundImage || ""
			}" style="max-width: 100px; display: ${
				button.backgroundImage ? "block" : "none"
			};" /><br>
              <button type="button" id="${buttonKey}-removeImage">Remove Image</button><br> 
              <label>Connection Type: 
                  <select name="${buttonKey}-connectionType">
                      <option value="tcp" ${
												button.connectionType === "tcp" ? "selected" : ""
											}>TCP</option>
                      <option value="udp" ${
												button.connectionType === "udp" ? "selected" : ""
											}>UDP</option>
                  </select>
              </label><br>
              <label>Connection Port: <input type="number" name="${buttonKey}-connectionPort" value="${
				button.connectionPort
			}"></label><br>
              <label>Connection Target: <input type="text" name="${buttonKey}-connectionTarget" value="${button.connectionTarget.join(
				","
			)}"></label><br>
              <label>Connection Command: <input type="text" name="${buttonKey}-connectionCommand" value="${
				button.connectionCommand
			}"></label><br>
          `;
			container.appendChild(div);

			var input = div.querySelector(
				`input[name="${buttonKey}-backgroundImage"]`
			);
...
document
	...
	.addEventListener("submit", function (event) {
		...
		Object.keys(config.buttonsSettings).forEach(buttonKey => {
			...
			config.buttonsSettings[buttonKey].backgroundColor = formData.get(
				`${buttonKey}-backgroundColor`
			);
			config.buttonsSettings[buttonKey].connectionType = formData.get(
				`${buttonKey}-connectionType`
			);
			config.buttonsSettings[buttonKey].connectionPort = Number(
				formData.get(`${buttonKey}-connectionPort`)
			);
			config.buttonsSettings[buttonKey].connectionTarget = formData
				.get(`${buttonKey}-connectionTarget`)
				.split(",");
			config.buttonsSettings[buttonKey].connectionCommand = formData.get(
				`${buttonKey}-connectionCommand`
			);
		});
